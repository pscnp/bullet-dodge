...
class BulletDodgeEnvPixel(gym.Env):
    def __init__(self):
        self.max_steps = 500
        ...
        # observation space:
        self.observation_space = spaces.Box(low=0, high=1, shape=(3, 2, self.map_size, self.map_size), dtype=np.uint8)
    ...
    def reset(self, seed=None):
        ...
        self.step_count = 0
        self.stack_obs = np.tile(self.get_observation(), (3, 1, 1, 1))

        return self.stack_obs, {}
    ...
    def step(self, action: int):
        ...
        # update stack observation at the end of each step() call
        self.stack_obs[:-1] = self.stack_obs[1:]
        self.stack_obs[-1] = self.get_observation()
        ...
        if is_collide or self.step_count == self.max_steps:
            if is_collide:
                ...
                truncation = True
                info['final_observation'] = self.stack_obs
            next_observation, _ = self.reset(seed=None)
        else:
            next_observation = self.stack_obs

        return next_observation, reward, termination, truncation, info

    def get_observation(self) -> npt.NDArray[np.uint8]:
        # clear surface
        self.surface.fill((0, 0, 0))

        # draw agent
        agent_x, agent_y = cartesian_to_pygame(self.agent_position[0], self.agent_position[1], self.map_size)
        pygame.draw.circle(self.surface, self.agent_color, (agent_x, agent_y), self.agent_r)

        # draw bullets
        if self.bullet_positions.size > 0:
            bullets_xy = [cartesian_to_pygame(x, y, self.map_size) for x, y in self.bullet_positions]
            for bullet_x, bullet_y in bullets_xy:
                pygame.draw.circle(self.surface, self.bullet_color, (bullet_x, bullet_y), self.bullet_r)

        pixel_obs = pygame.surfarray.array3d(self.surface)  # (map_length, map_length, 3)
        obs = pixel_obs.transpose((2, 0, 1))  # (3, map_length, map_length)
        obs = obs[:2, ...]  # (2, map_length, map_length)
        obs[obs == 255] = 1
